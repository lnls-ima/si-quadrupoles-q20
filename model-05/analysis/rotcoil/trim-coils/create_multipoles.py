...
import sys
from lnls.rotcoil import RotCoilMeas_SIQuadQ20Trim, MagnetsAnalysis
...
def multipole_trim(curr_main_coil):
    ...
    data = MagnetsAnalysis(
        RotCoilMeas_SIQuadQ20Trim, serials, curr_main_coil=curr_main_coil)
    data.init()
    ...
    for cidx in range(1, len(currents)):
        if abs(currents[cidx]) < 1:
            currents[cidx] = 0
        sys.stdout = open(
            'main_' + curr_str + 'A/MULTIPOLES-{:.0f}A.txt'.format(
                currents[cidx]), 'w')
